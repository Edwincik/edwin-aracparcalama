	...
	for k,v in pairs(Config.Places) do
        ...
        exports['qb-target']:AddCircleZone("carpartibus_sales", vector3(v.tped.x, v.tped.y, v.tped.z), 2.0,{
            ...
            options = {
                {
                    type = "client",
                    event = "edwin-vehicleshredding:client:openMenu",
					icon = "fas fa-shop",
                    label = label,
                },
            },
...
Citizen.CreateThread(function ()
	while true do
		if clientcooldown then
			Citizen.Wait(120000) -- Citizen.Wait(1800000) -- 1800000
...
RegisterNetEvent('edwin-vehicleshredding:client:openMenu', function()
            local chopShop = {
                {
                ...
                },
                {
                    ...
                    params = {
                        event = 'edwin-vehicleshredding:client:openChop',
                        args = {
...
RegisterNetEvent('edwin-vehicleshredding:client:openChop', function(data)
    QBCore.Functions.TriggerCallback('edwin-vehicleshredding:server:getInv', function(inventory)
            ...
            for i = 1, #data.items do
                if v.name == data.items[i].item then
                    chopMenu[#chopMenu + 1] = {
                        ...
                        params = {
                            event = 'edwin-vehicleshredding:client:chopitems',
                            args = {
...
RegisterNetEvent('edwin-vehicleshredding:client:chopitems', function(item)
    local sellingItem = exports['qb-input']:ShowInput({
     ...
     if sellingItem then
        ...
        if tonumber(sellingItem.amount) > 0 then
            TriggerServerEvent('edwin-vehicleshredding:server:sellItems', item.name, sellingItem.amount, item.price)
        else
...
function FinishedChopping()
	carDismantlingActive = false
	DeleteEntity(car)
	TriggerServerEvent("edwin-vehicleshredding:giveitem", class)
end
